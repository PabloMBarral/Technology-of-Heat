...
isolines = {
    'Q': np.linspace(0, 1, 2),
    'p': np.array([0.1, 1.01, 4.01, 13.01, 25.01, 66.01, 74.01]),
    'v': np.array([]),
...
diagram.draw_isolines(fig, ax, 'Ts', x_min=-2000, x_max=9500, y_min=-100, y_max=650)
...
ax.set_title('CHP Plant T-s Diagram', fontsize=20)

ax.tick_params(axis='x', labelsize=12)
ax.tick_params(axis='y', labelsize=12)
plt.tight_layout()

fig.savefig('chp_plant_ts_diagram.svg')

for cp in my_plant.comps['object']:
    plotting_data = cp.get_plotting_data()
    if plotting_data is not None:
        print(f"Componente {cp.label} tiene plotting data: {plotting_data}")
    else:
        print(f"Componente {cp.label} no tiene plotting data.")
"""
    ### NOTES ###
